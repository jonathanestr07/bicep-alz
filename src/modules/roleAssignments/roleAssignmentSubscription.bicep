...
param roleAssignmentNameGuid string = guid(subscription().subscriptionId, roleDefinitionId, assigneeObjectId)
...
module customerUsageAttribution '../CARML/customerUsageAttribution/cuaIdSubscription.bicep' = if (!telemetryOptOut) {
  name: 'pid-${cuaid}-${uniqueString(subscription().subscriptionId, assigneeObjectId)}'
  params: {}
}
